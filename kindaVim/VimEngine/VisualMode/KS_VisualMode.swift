...
extension KindaVimEngine {
    ...
    func handleVisualModeUsingKeyboardStrategy(for keyCombination: KeyCombination) {
        ...
        case .j, .controlJ, .controlN, .downArrow:
            post(ksVisualMode.j(times: count, state))
            endCurrentMove()
        case .k, .controlP, .upArrow:
            post(ksVisualMode.k(times: count, state))
            endCurrentMove()
...
extension KindaVimEngine {
    ...
    func parseOperatorCommandForVisualModeUsingKeyboardStrategy() {
        ...
        case [.g, .j], [.g, .downArrow]:
            post(ksVisualMode.gj(times: count, state))
            enterVisualMode()
        case [.g, .k], [.g, .upArrow]:
            post(ksVisualMode.gk(times: count, state))
            enterVisualMode()
        case [.g, .zero]:
